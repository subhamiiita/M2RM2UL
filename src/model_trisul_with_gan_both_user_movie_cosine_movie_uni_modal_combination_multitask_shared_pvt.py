...
losses = {
	"out_norm_rate":"MSE" ,
	"out_genre": "categorical_crossentropy",
    "out_movie_rate":"MSE",
    "out_user_rate":"MSE"
}

def make_model(m_size_text,m_size_video,m_size_audio, u_size, m_rate_size, u_rate_size,op_genre_size,window_size):
    def concat(x):
...
#     elu_audio= tf.keras.layers.ELU(alpha= 0.2)(gcn_layer_audio)
    ...
    #final concatenation
    shared_private= keras.layers.concatenate([d_m, d_u,elu_text])
    d_p1= keras.layers.Dense(units= int((m_size_text+u_size)/2), activation="tanh")(shared_private)
    d_p2= keras.layers.Dense(units=1024, activation="sigmoid")(d_p1)
    d_p3= keras.layers.Dense(units=256, activation="sigmoid")(d_p2)
    
    
    concated_1= keras.layers.concatenate([d_m, d_u,elu_text])
    d_c1= keras.layers.Dense(units= int((m_size_text+u_size)/2), activation="tanh")(concated_1)
    d_c2= keras.layers.Dense(units=1024, activation="sigmoid")(d_c1)
    d_c3= keras.layers.Dense(units=256, activation="sigmoid")(d_c2)

    shared_rating= keras.layers.concatenate([d_c3, d_p3])
    d_c4= keras.layers.Dense(units=128, activation='sigmoid')(shared_rating)
    d_c5= keras.layers.Dense(units=1, activation="sigmoid", name= "out_rating")(d_c4)
    
    
    concated_2= keras.layers.concatenate([d_m, d_u,elu_text])
    d_d1= keras.layers.Dense(units= int((m_size_text+u_size)/2), activation="tanh")(concated_2)
    d_d2= keras.layers.Dense(units=1024, activation="sigmoid")(d_d1)
    d_d3= keras.layers.Dense(units=256, activation="sigmoid")(d_d2)

    shared_genre= keras.layers.concatenate([d_p3, d_d3])
    d_d4= keras.layers.Dense(units=128, activation='sigmoid')(shared_genre)
    d_d5= keras.layers.Dense(units=op_genre_size, activation="sigmoid", name= "out_genre")(d_d4)
    
    model= keras.models.Model([ip_m_text, ip_m_video,ip_u], [d_c5,d_d5, d_r_op_m, d_r_op_u])
