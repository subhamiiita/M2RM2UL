...
#     elu_audio= tf.keras.layers.ELU(alpha= 0.2)(gcn_layer_audio)
    ...
    #final concatenation
    concated= keras.layers.concatenate([d_m, d_u,elu_text,elu_video,elu_user])
    
    d_c1= keras.layers.Dense(units= int((u_size+m_size_text)/2), activation="tanh")(concated)
    d_c2= keras.layers.Dense(units=516, activation="sigmoid")(d_c1)
    ...
    d_d1= keras.layers.Dense(units= int((u_size+m_size_text)/3), activation="tanh")(concated)
    ...
    model= keras.models.Model([ip_m_text, ip_m_video,ip_u], [d_c4,d_d4, d_r_op_m, d_r_op_u])
